...
void parse_ir(const char *file) {
    ...
    while (fgets(line, sizeof(line), f)) {
        // 1. Handle Globals (Strings and i32)
        if (line[0] == '@') {
            Global g;
            if (strstr(line, "c\"")) {
        ...
        // 3. Handle Constants (i32, f32, f64)
        if (strstr(line, "const")) {
            in.kind = INS_CONST;
            char ty[16];
            // We use a double for the immediate to capture floats/ints safely
            double val; 
            sscanf(line, " %%%d = const %15s %lf", &in.dst, ty, &val);
            
            if (strcmp(ty, "f32") == 0) { in.type = TY_F32; in.imm = (int)val; } // Note: bit-cast needed for true float
            else if (strcmp(ty, "f64") == 0) { in.type = TY_F64; in.imm = (int)val; }
            else { in.type = TY_I32; in.imm = (int)val; }
        } 
        
        // 4. Handle Math Operations (add, sub, mul, div, mod)
        else {
            char op[16], ty[16];
            ...
            // 5. Handle Control Flow & Specials
        ...
        if (in.kind != 0 || in.dst != -1 || in.label[0] != '\0') {
            ...
            free(in.src_args); // Clean up unused allocations
        }
    }
    fclose(f);
}
